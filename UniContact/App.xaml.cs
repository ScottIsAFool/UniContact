...
namespace UniContact
{
    public partial class App
    {
        ...
        {
            get { return (AuthenticationService)Current.Resources["AuthenticationService"]; }
        }

        public static AppSettings Settings
        {
            get { return (AppSettings) Current.Resources["Settings"]; }
        }
            ...
            if (continuationArgs != null)
            {
                ...
                if (brokerArgs != null)
                {
                    if (brokerArgs.WebAuthenticationResult.ResponseStatus == WebAuthenticationStatus.Success)
                    {
                        ...
                        {
                            var queryString = url.Replace(Constants.InstagramReturnUrl, string.Empty);
                            Messenger.Default.Send(new NotificationMessage(queryString, Constants.Messages.InstagramAuthMsg));
                        }
                        else if (url.Contains("googleplus"))
                        {
                            var queryString = url.Replace(Constants.GoogleReturnUrl, string.Empty);
                            Messenger.Default.Send(new NotificationMessage(queryString, Constants.Messages.GooglePlusAuthMsg));
                        }
                        else if (url.Contains("foursquare"))
                        {
                            var queryString = url.Replace(Constants.FourSquareReturnUrl, string.Empty);
                            Messenger.Default.Send(new NotificationMessage(queryString, Constants.Messages.FourSquareAuthMsg));
                        }
                    }
                }
            }
        ...
        {
            var settings = SimpleIoc.Default.GetInstance<IApplicationSettingsService>();
            settings.Set(Constants.Settings.DownloadDisplayPictures, Settings.DownloadDisplayPictures);
            settings.Save();
        }
